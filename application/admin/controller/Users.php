...
class Users extends Base
{
    ...
    public function index(){
        if (\request()->isPost()){
            $min = !empty($_POST["logmin"]) ? strtotime($_POST["logmin"]) : strtotime(date('Y-01-01'));
            $max = !empty($_POST["logmax"]) ? strtotime($_POST["logmax"]) : strtotime(date('Y-m-d',strtotime('+1 day')));
            $keyword = isset($_POST['keyword']) ? $_POST['keyword'] : '';
            $where = null;
            if (!empty($keyword)) {
                $where["id|tel|invite"] = array("like", "%$keyword%");
            }
            if(isset($_POST['do'])){
